Jahcoin Core version *0.15.1* is now available from:

  <https://download.jahcoin.org/jahcoin-0.15.1/>
...
A number of changes to the way Jahcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 gekoins per kB (0.00001 JAHC). This is to prepare for dropping the minimum transaction fee to 0.1 gekoins per kB (0.0001 JAHC) in the next release.
